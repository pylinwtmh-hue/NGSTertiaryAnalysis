...
profiles {
    ...
    dgx {
        params {
            ...
            // Pangolin GPU：V100 為 compute 7.0（sm_70）。
            //   ⚠️ 容器裡的 PyTorch 必須是 **CUDA 12.x 且含 sm_70 kernel** 的 build。
            //      DGX-2 驅動只到 CUDA 12.2（driver 535.x），而 CUDA 13.x 既要求驅動
            //      r580+、又已在工具鏈層面移除 Volta 支援 → cu130 的 torch 會噴
            //      "The NVIDIA driver on your system is too old (found version 12020)"。
            //      pangolin_1.0.0.sif 已 pin 到 torch 2.5.1+cu121，%test 會在 build
            //      階段擋掉不合格的 build（詳見 DEVELOPMENT_NOTES.md 的容器建立章節）。
