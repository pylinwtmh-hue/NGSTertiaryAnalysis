...
profiles {
    ...
    dgm {
        params {
            ...
            //   DGM 的卡兩顆容器都跑得動：cu121 的 arch list 是 sm_50…sm_90，
            //   含 sm_86（RTX A2000 / Ampere）；若實際是 RTX 2000 Ada（sm_89）也可用
            //   （cubin 在同一 major 版本內向前相容，sm_86 → sm_89）。
            //   ⚠️ 型號待確認：README 硬體表寫 RTX 2000 Ada，開發筆記記 RTX A2000，
            //      兩者名字很像但不同世代。結論不受影響，但硬體表要正確。
