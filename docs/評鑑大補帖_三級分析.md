...
profiles {
    ...
    dgx {
        params {
            ...
            //   ⚠️ pangolin_1.0.0.sif 是**一顆通吃兩台機器**的容器，torch 的 wheel
            //      必須同時含 sm_70（DGX-2 V100）與 sm_120（開發機 RTX PRO 6000
            //      Blackwell）。只有 CUDA 12.8/12.9 的 build 有可能兩者兼具：
            //        ≤12.6 沒有 sm_120；13.0 起移除了 Volta/sm_70。
            //      已 pin 到 torch 2.7.0+cu128；缺任一 arch 時 %test 會讓 build 失敗
