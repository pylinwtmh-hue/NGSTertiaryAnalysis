...
| # | 位置 | 症狀（VAL-10） | 原因 | 修正 |
|---|------|--------------|------|------|
| 8 | `COMBINE_DRAGEN`（3.6） | PASS 變異可能從報告消失；或 DRAGEN 濾掉的 allele 被拼進報告裡的 MNV | combine 對整份 hard-filtered VCF 做，合成紀錄的 FILTER 沿用 anchor，`ADD_DRAGEN_TAG` 只收 PASS：anchor 是 non-PASS 時整筆被丟、anchor 是 PASS 時 non-PASS 的 allele 被併入 | 非 chrM 只拿 PASS 進 combine（non-PASS 本來就會被丟）；toy 驗證改完後的報告 = DRAGEN 的 PASS 集合，只多了 PASS+PASS 合成的 MNV |
