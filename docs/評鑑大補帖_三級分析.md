...
profiles {
    ...
    dgm {
        params {
            ...
            pangolin_num_gpus   = 1
            // DGM 與開發機共用同一顆（cu128）。依據：DGM 原本跑舊的 cu130 容器就是正常的，
            //   而 cu128 的 arch list（sm_75…sm_120）涵蓋範圍比 cu130（sm_75 sm_80 sm_90
            //   sm_100 sm_120）還寬，需要的驅動版本又更舊（12.x 不必 r580+）
            //   → 跑得動 cu130 就一定跑得動 cu128。
            //   只有 DGX-2 的 V100（sm_70）需要另一顆 cu121 容器。
            pangolin_sif        = "pangolin_1.0.0.sif"
