...
（選代表轉錄本 → 攤平成 75 欄 TSV）
   │
acmg_classifier.py
（ACMG/AMP 2015 + ClinGen SVI → 82 欄）
   │
...
輸出的 82 欄 TSV 中，ACMG 相關欄位不只給結論，還給**理由**：
...
| 目錄 | 內容 |
|------|------|
| `00_prepare` | 送進 VEP 的 VCF `<sample>.snv_for_annotation.vcf.gz`（norm、caller tag；DRAGEN 另含 combine、chrM 分流出的 `<sample>.mito_for_annotation.vcf.gz`，以及倍體 QC `<sample>.ploidy_qc.txt`） |
| `01_vep` | VEP 註解後的 VCF `<sample>.vep.vcf.gz` |
| `02_pangolin` | 加上 Pangolin 剪接分數的 VCF `<sample>.pangolin.vcf.gz` |
| `03_acmg` | **`<sample>.snv_indel.acmg.tsv`（82 欄，主要交付物）**；`parse_vep_csq.py` 的 75 欄 TSV 是中間檔，不另外發布 |
| `04_mito` | 粒線體變異 `<sample>.mito.tsv`（＋ `<sample>.mito.vep.vcf.gz`） |
| `05_str` | STR 判讀 `<sample>.str.tsv` |
| `06_cnv_sv` | AnnotSV 註解後的 CNV / SV TSV |
| `07_pgx` | `<sample>.pgx.tsv`（藥物基因型 + 建議）及 PharmCAT / OptiType / StellarPGx 原始結果 |
| `pipeline_info` | 在 `out_dir` 根目錄（不在各樣本資料夾內）：執行報告 / timeline / trace |
...
**背景**：二級 ensemble 的 `FILTER` 是 `bcftools merge` 把兩個 caller 的 FILTER
合併而來，**不代表「哪個 caller 有 call」**。

**如果照直覺用 `FILTER="PASS"` 過濾會怎樣？**

> **HC-only 的變異會被丟掉** —— 它們的 FILTER 常常帶著 DV 的值，不是 PASS。

這違反 ensemble 的基本原則（「只要有一個 caller 叫到就保留，由下游判讀」）。

**做法**：`add_callers_tag.py` 依 **GT 欄位**判斷每個 caller 的實際 call 狀態，
...
`CALLERS` 有四種值：`DV+HC` / `DV` / `HC` / **`NONE`**（兩邊都沒有 ALT genotype）。
上面的條件不收 `NONE` —— 所以「兩邊都沒 call」的紀錄會被擋掉。

### ⚠️ 曾經的缺陷：兩邊都沒 call 被標成 HC（2026-09 修正）

跟 10.12 的 MT-RNR1 一樣，這是實測時抓到的**真實缺陷**。起因是與 DRAGEN 比對
SUZ12 時，NCKUH 多出一個 DRAGEN 沒有的變異。

**舊版的 `determine_callers()`**：

```python
if dv_called and hc_called: return "DV+HC"
elif dv_called:             return "DV"
else:                       return "HC"      # ⚠️ 兩邊都沒 call 也走到這裡
```

**沒有 `NONE`**。DV `./.` 或 `0/0`、HC 也沒 call 的紀錄，全部被標成 `"HC"`，
通過上面的過濾，以 ZYGOSITY `ref` / `unknown` 的「幽靈列」出現在 ACMG 表。

更糟的是 `prepare_vcf.nf` 的註解**正好描述了這個情況、結論卻是錯的**：
「DV=0/0, HC=./. …CALLERS 被判為 HC …實際上這種 case 不會有 CALLERS tag」
—— tag 其實一律會寫。**本文件的舊版也照抄了這個錯誤結論**（「`CALLERS` 有值 ⟺
至少一個 caller 有 ALT call」），沒有回頭核對程式碼。

**SUZ12 `chr17:31998950` 實際發生的事**（已用 pysam 內建的 bcftools 逐欄重現）：

| 步驟 | 狀態 |
|------|------|
| DV | delinsTT 的三個片段（`GAAA>GAA`、952 A>T、953 A>T）**全判 RefCall**（`./.`） |
| HC | 叫出片段、phase、combine_phased 正確合成 `GAAA>GTT`（`0\|1`、`COMBINED=2`） |
| 二級 `merge --merge all` | 同一 POS 的 DV `GAAA>GAA` 與 HC `GAAA>GTT` 被併成**一筆多等位** `GAAA GAA,GTT`，FILTER 變成 DV 的 `RefCall` |
| 三級 `norm -m -any` | 拆回四筆：950 `GA>G`（DV `./.` + HC `0\|0`）、951 `AAA>TT`（HC `0\|1`）、952/953 `A>T`（兩邊 `./.`） |
| 舊版 `determine_callers` | **四筆全部標 `HC`** → 全部進 ACMG 表 |

結果：報告多出錯誤的 `c.2170del`（還帶著 DV 否決那個候選的 `AD 10,14`、VAF 0.583，
看起來非常可信），正確的 `c.2168_2170delinsTT` 反而帶著 `AD 10,0` —— 看起來像
artifact（這是另一個 bug，見 3.5 的 `get_ad()`）。**兩列各自都會誤導判讀，而且方向最糟。**

**修正（三處，彼此獨立的防線）**：

| # | 位置 | 改動 |
|---|------|------|
| 1 | 三級 `determine_callers()` | 補上 `elif hc_called` 與 `else "NONE"` |
| 2 | 三級 `get_ad()` | 個別缺值保留 `.`（`10,.`），不再補 0 |
| 3 | 二級 `BCFTOOLS_ENSEMBLE` | DV arm 在 `norm -m -any` 之後只留 `GT="alt"`，DV 否決的候選根本不進 merge |

修正後 SUZ12 這個位點只剩 delinsTT 一筆、`FILTER=PASS`、DV 欄位誠實地是 `.`、
HC 欄位是 `DP 5 / AD 3,2`。`scripts/test_add_callers_tag.py` 守住這個案例
（見 12.5）。

**連帶影響**：舊版 stderr 的「HC only」統計用的是同一個 `else`，把 `NONE` 也算進去。
舊文件引用的「**NA12878_WES HC-only 23.9%（8,897 / 37,198）**」**被灌水了**，
新版 stderr 會把 `NONE` 單獨列出；修正後的實測見 3.3.1（VAL55 WGS：HC only 7.4%）。

### 3.3.1 VAL55 實跑驗證與兩個後續修正（2026-09）

**實跑結果**（VAL55 WGS、男性、`--academic_dbnsfp`）：兩邊都沒 ALT 的「幽靈列」= **0**；
ACMG 表沒有 `NONE`、沒有 `unknown`；SUZ12 只剩 delinsTT 一列。ADD_CALLERS_TAG 的 stderr：
DV+HC 89.0%、DV only 3.0%、**HC only 7.8%**、NONE 0.2%（修正後依染色體確認：全部在男性 chrX/Y，
是被 `+fixploidy` 截成 REF 的 het）。

⚠️ 這次二級用的 Parabricks 版本和上次不同：SUZ12 的 DV 讀數完全相同（DP 24、AD 10,14），
但 GQ 9→15、由 RefCall 翻成 het → 兩個 caller 都 call 到（`CALLERS=DV+HC`），**已不是當初
出錯的情境**。修正有效的證據是「幽靈列 = 0」＋ 12.5 的重現測試，不是這一列。

同一次驗證抓到兩個新問題（都已修正）：

| # | 位置 | 症狀（VAL55） | 原因 | 修正 |
|---|------|-------------|------|------|
| 4 | `combine_phased.py`（二級、三級同一支） | ensemble 仍有 28,050 筆 `FILTER=RefCall`，全部帶 `COMBINED`；報告裡 23,023 個變異被拆成「DV 一列 + HC 一列」，DV 那列的深度是別人的 | DV 否決的候選（`./.`）參與叢集，被挑成 anchor（足跡最寬）→ 合成紀錄帶著它的 FILTER/AD/VAF，POS 被撐寬、和 HC 對不上 | GT 沒有 ALT 的紀錄不進叢集（二級大補帖 7.6.1） |
| 5 | `parse_vep_csq.py` `infer_zygosity()` | 15,422 列 `ZYGOSITY=ref`，全是 `CALLERS=HC`、`GT_DV=0/0` 或 `0\|0` | 只要 DV 的 GT 不是 missing 就用 DV 的；DV、HC 在同一 POS call 到不同 allele 時，HC 那個 allele 的 DV 欄是 `0/0` | 用「真的 call 到 ALT」的 caller 的 GT（與 `is_called()` 同定義），見 5.4 |

**修正後重跑（VAL55）**：ensemble 的 RefCall 28,050 → **0**；「同一變異拆兩列」23,023 → **820**；
`ZYGOSITY=ref` 15,422 → **0**；ADD_CALLERS_TAG 變成 DV+HC **89.9%**、DV only **2.6%**、
HC only **7.4%**（這才是 VAL55 WGS 真實的 HC-only 比例）、NONE 8,656；SUZ12 不變。
剩下 820 個（已修，見下）：抽查 5 例都是兩個 caller 對同一變異的寫法不同：combine 合成時叢集範圍從第一個成分的 POS 開始，成分若是 indel 就帶著它的前導鹼基，而合成結果沒有再最小化；DV、HC 拆成分的方式不同 → 前導鹼基長度不同 → POS 不同，merge 合不起來，三級 norm 修剪後才一樣。兩邊各自最小化後 5 例完全相同。

同一次驗證的最後兩項（2026-09 已修，VAL55 重跑確認）：

| # | 位置 | 症狀（VAL55） | 原因 | 修正 |
|---|------|-------------|------|------|
| 6 | `combine_phased.py` | 修完第 4 項後仍有 820 個變異拆成兩列 | 合成結果沒有最小化，DV、HC 的前導鹼基長度不同 → POS 不同 | 輸出前先最小化（二級大補帖 7.5） |
| 7 | 二級 `BCFTOOLS_ENSEMBLE` → 三級 `HAPLOID_HET` 欄 | NONE 8,656 筆全在男性 chrX（2,607）/ chrY（6,049）；另有數百筆 chrX het 被顯示成 hemizygous | `+fixploidy` 只留第一個 allele，het 依順序（phasing 後是任意的 phase 方向）變成 REF 或 ALT | 男性 chrX 非 PAR 的 het 一律保留成 ALT 並標記 `HAPLOID_HET` 給人工複核；chrY 的 het 不進報告（二級大補帖 12.5） |

第 7 項的理由：男性的 het 多半是比對假象，但也可能是 47,XXY 或**體細胞嵌合** —— X-linked 顯性、
男性通常致死的疾病（IKBKG、MECP2、CDKL5、PORCN、OFD1）男性病人常是嵌合，PCDH19 更是只有嵌合男性
發病，所以不能一律藏掉。`HAPLOID_HET` 有值的列，ZYGOSITY 仍是 hemizygous，請看 VAF 與位置判斷。

**重跑確認（VAL55）**：拆兩列 820 → **2**（重複序列 indel 的左對齊差異）；NONE 10,159 筆**全在 chrY**；
帶 `HAPLOID_HET` 的列**全在 chrX**（5,735 列）；HC only 7.3%；SUZ12 不變。
粒線體不動：ensemble 裡 chrM 的 het 仍會被截斷，實驗室以 `04_mito` 報告為準（2026-09 決定）。

### 3.3.2 DRAGEN 女性樣本驗證（VAL-10，2026-09）

新版確認：82 欄、`HAPLOID_HET` 全為 `.`、合成紀錄都是最小表示、`AD_DV` 有值 5,940,465 / 5,940,563
（與 7 月相同）、`CALLERS` 全為 `DRAGEN`。女性核對：ploidy QC declared / estimated 都是 `XX`、無警示；
chrX 的 GT 全是雙套（沒有單套 `1`）、chrY 0 筆；chrX 的 STR（DMD、AR、ZIC3、SOX3、FMR1、AFF2）都有兩個 allele。
同一次驗證抓到兩個 DRAGEN／女性才會碰到的問題（已修正）：

| # | 位置 | 症狀（VAL-10） | 原因 | 修正 |
|---|------|--------------|------|------|
| 8 | `COMBINE_DRAGEN`（3.6） | 23 叢、35 筆 PASS 變異從報告消失；120 叢、131 筆 DRAGEN 濾掉的紀錄被拼進報告裡的 MNV（`TargetedConflict`、`LowDepth`、`MosaicLowAF`） | combine 對整份 hard-filtered VCF 做，合成紀錄的 FILTER 沿用 anchor，`ADD_DRAGEN_TAG` 只收 PASS：anchor 是 non-PASS 時整筆被丟、anchor 是 PASS 時 non-PASS 的 allele 被併入 | 非 chrM 只拿 PASS 進 combine（non-PASS 本來就會被丟）；toy 驗證改完後的報告 = DRAGEN 的 PASS 集合，只多了 PASS+PASS 合成的 MNV |
| 9 | `parse_vep_csq.py` `infer_zygosity()`（5.4） | 女性 chrX 沒有任何 `hom`：GT `1/1`、`1\|1` 的 73,668 筆都顯示 `hemizygous` | 舊版在 chrX/chrY 上只要兩個 allele 都是 ALT 就標 hemizygous，不看 GT 是單套還是雙套（男性 PAR 的 `1/1` 同樣被標錯） | 看 GT 的套數：單套 → `hemizygous`，雙套 `1/1` → `hom`（套數已由 `+fixploidy` / DRAGEN 依性別決定） |

第 8 項從 7 月加入 DRAGEN combine 起就存在，不是 9 月的改動造成的；9 月的 no-ALT 修正（第 4 項）只擋掉
「沒有 ALT」的紀錄，擋不到「有 ALT 但被 DRAGEN 濾掉」的。

第 8、9 項修正後重跑，再用唯讀診斷腳本（和 `COMBINE_DRAGEN` stderr 數字完全一致）逐叢檢查，又找到兩項：

| # | 位置 | 症狀（VAL-10） | 原因 | 修正 |
|---|------|--------------|------|------|
| 10 | `combine_phased.py`（二級三級同一支，二級大補帖 7.6.2） | 104,277 個合成中 9,299 個把 phase 未知的 het 當成同一條單體；**7,927 個 PASS allele 消失**、1,435 叢寫出沒人 call 的 allele | 「重疊一律合」＋ 未 phase 的 GT 依位置都落在同一條單體；缺失範圍內的 SNV、包在大缺失裡的小缺失被吃掉，重疊缺失被併成更長的缺失 | phase 未知就不合；重疊只准落在沒改變的錨定鹼基，否則整叢原封通過；同一位置先套 SNV 再套 indel |
| 11 | `ADD_DRAGEN_TAG`（3.6） | 報告出現 `ZYGOSITY=ref` 的假列，例 CYP21A2 `c.293-13C>G`（In2G 的位置） | DRAGEN targeted caller 寫 `C>G,A 2/2`（樣本是 A/A），`norm -m -any` 拆出 `C>G 0/0`；DRAGEN 路徑沒有 NCKUH 的 `CALLERS=NONE` 那道關 | norm 之後非 chrM 只留 `GT="alt"`（與 `is_called()` 同定義），chrM 全部保留 |

第 11 項的那一列一度被誤讀成「VAL-10 帶 In2G」：GT 是 `0/0`，樣本其實是 `c.293-13C>A` 的 A/A（Benign）。
 ...
 *   ★ 設計意圖一直是對的，壞的是實作 —— 檔頭說明對、程式碼錯，
 *     正是「註解不能代替測試」的例子（見 3.3、12.5）。
...
process FILTER_FOR_ANNOTATION {
    ...
    script:
    """
    # ★★ 以下這段註解是整個檔案最重要的內容 —— 它記錄了兩個陷阱：
    #    (1) 照直覺用 FILTER=PASS 過濾會靜默丟掉 HC-only 的變異；
    #    (2) 舊版註解自己宣稱的「兩邊都沒 call 會被擋掉」其實沒生效（見 3.3）。★★
    #
    ...
    #   ★ 舊版這裡有一張「FILTER=PASS → DV 有 call / RefCall → DV 叫 0/0」的對照表。
    #     那張表只在 DV 與 HC 的 allele 完全相同時成立；兩者 allele 不同時，
    #     merge 會把 DV 的 RefCall 蓋到 HC 真正的 call 上（SUZ12 就是這樣）。
    #
    ...
    #   ★ 評鑑時可以講的教訓：舊註解把「設計意圖」寫成「已驗證的事實」，
    #     而且連本文件的舊版都照抄了。現在由 test_add_callers_tag.py 讀這一行的
    #     實際 -i 條件來驗證 NONE 不在允許清單裡（見 12.5）。
    ...
    bcftools view \\
        ...
        #  -i = include（只保留符合條件的）
        #  三個條件用 || 串起來，等同於「CALLERS 不是 NONE」
        #  （舊版這裡寫「等同於 CALLERS 欄位有值」—— 在沒有 NONE 的年代，
        #   CALLERS 永遠有值，所以這個條件其實什麼都沒擋）
...
def determine_callers(variant, dv_idx: int, hc_idx: int) -> str:
    ...
    （完整 docstring 見原始檔；重點：四種情況都要明確處理，
      不能讓「兩邊都沒 call」掉進 else 當成 HC。）
    """
    ...
    if dv_called and hc_called:
        ...
        return "NONE"
    # ★★ 2026-09 修正。舊版是三分支：`if DV+HC / elif DV / else "HC"`。
    #
    # ⚠️ 本文件舊版在這裡寫過一段「這看起來像 bug，但實務上不會發生」的註解，
    #    理由是「ensemble VCF 只保留至少一個 caller 有記錄的位點，而 bcftools
    #    merge 不會產生兩邊都是 0/0 的列」。**這個推論是錯的**，錯在兩處：
    #    (1)「有記錄」≠「有 ALT call」—— DeepVariant 的 RefCall 就是一筆記錄
    #        （GT ./. 或 0/0）。DV RefCall + HC 沒有紀錄，在 ensemble 裡很常見。
    #    (2) 就算 merge 本身不產生 0/0，三級 norm -m -any 拆開多等位時，每一筆
    #        拆出來的紀錄對「沒帶那個 allele 的 caller」都會變成 0/0。
    #    SUZ12 一個位點就有 3 筆這樣的紀錄被標成 HC、進了 ACMG 表（見 3.3）。
    #
    # ★ 評鑑時可以講的教訓：「看起來像 bug、但我推論它到不了」的分支，
    #   推論本身必須被測試驗證，不能只寫成註解。這裡的推論被寫進程式碼註解、
    #   又被照抄進本文件，兩次都沒有人（包括寫這份文件的過程）去測它。
    #   現在由 test_add_callers_tag.py 直接測「兩邊都沒 call → NONE」（見 12.5）。
...
def get_ad(variant, sample_idx: int) -> str:
    ...
    **個別 missing 的元素保留為 "."**（例如 "10,."），不可補 0。
    ...
    try:
        ...
        #          ↑ 2026-09 修正：個別 missing 保留 "."。
        #            舊版是 str(max(int(v), 0))，把 missing 補成 0；本文件舊版還替它
        #            辯護說「寫 0 表示沒有 read 支持，語義上最接近 missing」。
        #            **這兩件事完全不同**：
        #              0   = 這個 caller 看了，0 條 reads 支持 → 審閱者會當 artifact
        #              "." = 這個 caller 對這個 allele 沒有資料 → 要去看另一個 caller
        #            SUZ12 的 delinsTT：DV 從未把 GTT 當成候選，AD 是 "10,."；舊版
        #            寫成 "10,0"，讓一個真的 het frameshift 看起來 VAF=0。
        #            元素數仍與等位基因數一致（"." 佔一格），不影響格式。
        #            下游只有 parse_vep_csq.py 讀它，且只是原樣傳進 TSV。
        return ",".join(cleaned)
    except Exception:
        return "."
...
def add_callers_tag(input_path: str, sample_id: str, output_path: str):
    ...
    for variant in vcf_in:
        ...
        if callers == "DV+HC":
            ...
            n_none += 1
        #  ★ 舊版這裡也是 else → n_hc_only，跟 determine_callers 同一個錯：
        #    NONE 被算進 HC only，於是 stderr 的 HC-only 百分比被灌水。
    ...
    # ── 統計摘要 ─────────────────────────────
    #   ★ 這幾行是驗證這一步是否正常的主要依據。
    #     ⚠️ 舊版實測的「NA12878_WES：DV+HC 約 70%、DV-only 約 6%、HC-only 約 24%」
    #        中，HC-only 包含了 NONE（見上方統計區塊），這組數字被灌水了。
    #        修正後 NONE 會單獨一行；二級加上 DV 非 call 過濾後，NONE 也會大幅減少。
    #        修正後實測（VAL55 WGS，2026-09）：DV+HC 89.9%、DV only 2.6%、HC only 7.4%、NONE 0.2%。
...
process ADD_DRAGEN_TAG {
    ...
    #   ★ 為什麼要在 norm「之後」濾：拆開前 C>G,A 2/2 整筆有 ALT（A），GT="alt" 會留下；
    #     要拆成一個 allele 一列之後，才看得出 C>G 那列這個人其實沒有帶。
...
//      ★ 這條規則的精神：combine 只改變「PASS 變異怎麼寫」，不改變「哪些變異會出報告」。
// ──────────────────────────────────────────────────────────────
process COMBINE_DRAGEN {
    ...
    #   ★ FILTER="." 是「沒有 FILTER 值」；cyvcf2（add_dragen_tag.py）把它和 PASS 一樣當成通過，
    #     這裡要用同一個定義，否則兩邊對「PASS」的認知不同。
...
workflow PREPARE_VCF_DRAGEN {
    ...
    dragen_ch   // tuple val(sample_id), path(dragen_vcf)（不需要 tbi）
                //   ★ 不需要 tbi：bcftools view、combine_phased.py 都是循序讀檔（不做隨機存取），
...
實測（81 欄版本時量的）：TSV 中有 in-silico 分數的變異約占 **1/4**。
...
輸出 **75 欄** TSV（`acmg_classifier.py` 再加 7 欄變成 82 欄）。
...
#   1. 兩個 caller 其中一個沒 call（用有 call 的那個的 GT）
#   2. 半缺失（1/.；防禦性處理，見下）
#   3. 性染色體的半合子（hemizygous）—— 看 GT 的「套數」（單套），不是只看在不在 chrX/chrY
#   4. 複合雜合（1/2 —— 兩個不同的 ALT，是 het 不是 hom）
#
# 【GT 是什麼】GT（genotype）= 這個人在這個位置的兩份（男性 chrX/Y 為一份）染色體各帶哪個 allele：
#   0 = REF、1 = 第一個 ALT、. = 沒判定；「/」未定相、「|」已定相（phased）。
#   0/1 → het、1/1 → hom、0/0 → 沒有變異、1（單套）→ hemizygous。ZYGOSITY 就是從 GT 推出來的。
# ──────────────────────────────────────────────────────────────
...
def _gt_called(gt: str) -> bool:
    ...
    if any(a == "." for a in alleles):   # 半缺失（./1）不算 call，與 is_called 一致
        return False
...
def haploid_het_callers(flag: str, callers: str) -> str:
    ...
    #   【為什麼要標記而不是直接藏掉？】男性單倍體區的 het 多半是比對假象，但也可能是
    #     體細胞嵌合或 47,XXY —— X-linked 顯性、男性通常致死的疾病（IKBKG、MECP2、CDKL5…）
    #     男性病人常是嵌合，PCDH19 更是只有嵌合男性發病（見二級大補帖 12.5）。
    if not flag or flag == ".":
        return "."
...
def infer_zygosity(gt_dv: str, gt_hc: str, chrom: str) -> str:
    ...
    #   【舉例】GT_DV=0|0、GT_HC=1/0（chr1:83829，DV 與 HC call 到不同長度的 indel）
    #     舊：取 DV 的 0|0 → "ref"（錯）；新：DV 沒 call、HC 有 → 取 1/0 → "het"
    if gt in (".", "./.", ".|.", ""):
        return "unknown"
    ...
    #   ★ 更正（2026-09 實測）：bcftools norm -m -any 預設（--multi-overlaps 0）會把 1/2
    #     拆成 1/0 和 0/1，**不會**產生 1/.；半缺失只在 --multi-overlaps . 或其他工具時出現。
    #     這一行是防禦性寫法，保留無害。
    ...
    #      ↑ 體染色體不會有單套 GT（防禦性寫法，沿用舊行為當 het）
    #   ★ 為什麼不直接讀樣本性別？因為不需要：GT 的套數已經是「caller 依性別決定的結果」，
    #     而且男性的 PAR 本來就是雙套 —— 只看性別反而會把 PAR 標錯。
...
# ──────────────────────────────────────────────────────────────
# 輸出欄位定義（75 欄；2026-09 加了最後一欄 HAPLOID_HET）
#
# ★★ 欄位順序的鐵則：新欄位一律加在「最後面」★★
...
| 檢查 | 期望值 | 意義 |
|------|--------|------|
| 欄位數 | **75** | `head -1 *.full.annotated.tsv \| awk -F'\t' '{print NF}'` |
...
| ClinGen 命中數 | 依樣本而異 | 實測 NA12878 WES 77 筆、VAL-10（WGS，2026-09）94 列有 VCEP 判讀 |
...
讀取 parse_vep_csq.py 產生的 56 欄 TSV，
  ⚠️ 註解說 56 欄，實際上現在是 75 欄（2026-09 加了 `HAPLOID_HET`）。程式不看欄位數（用欄位名查找），
     所以不影響行為，但註解該更新。
...
def process_tsv(
    ...
    if pvs1_counts:
        ...
        #   實測 NA12878 WES：PVS1 = 40、PVS1_Strong = 5；VAL-10（WGS，2026-09）：PVS1 = 1、PVS1_Strong = 2。
        #   （舊版這裡誤寫成 VAL-10；40/5 其實是 NA12878 WES 的數字，見 DEVELOPMENT_NOTES v3.6 驗證。）
...
使用範例：
  ...
  ⚠️ 這個欄位範圍已過時（當時是 60 欄，現在 82 欄）。
     現在應該用 cut -f76-82 或直接用欄位名。
        """
    )
    parser.add_argument("--input",  "-i", required=True,
                        help="輸入 TSV（支援 .gz）")
...
process ACMG_CLASSIFY {
    ...
    //   ★ 這是唯一「一定會發布」的 SNV 產物 —— 82 欄 TSV，交付給 GUI 與判讀者。
...
| 篩出需人工複核 | `awk -F'\t' '$80=="DIFFER"'`（`CLINGEN_AGREEMENT` 欄） |
| 篩出降級的 LoF | `awk -F'\t' '$81 ~ /PVS1_/ && $81 != "PVS1"'`（`PVS1_STRENGTH` 欄） |

> 欄號是 2026-09 加了 `HAPLOID_HET`（欄 75）之後的；之後再加欄就會再位移，
> 寫腳本請改用欄位名查欄號：
> `col() { head -1 "$TSV" | tr '\t' '\n' | grep -nx "$1" | cut -d: -f1; }`，
> 例如 `awk -F'\t' -v c=$(col CLINGEN_AGREEMENT) '$c=="DIFFER"' "$TSV"`。
...
五支測試腳本，全部只用 Python 標準庫，不需要真實資料。
共同原則：**每一支都對應一個已經修過的嚴重 bug**，測試存在的目的是讓那個 bug 不會回來。

| 測試 | 對象 | 涵蓋 | 守住的 bug |
|------|------|------|-----------|
| `test_combine_phased.py` | `combine_phased.py` | **26 個案例** | 合成記錄 AD 消失、否決候選被當 anchor、合成結果未最小化、phase 未知的重疊 het 被合成（二級大補帖 7.6 / 7.6.1 / 7.5 / 7.6.2） |
| `test_parse_dragen_ploidy.py` | `parse_dragen_ploidy.py` | 解析 + 判定 + 警告 | — |
| `test_parse_vep_csq.py` | `parse_vep_csq.py` | STRAND_BIAS 門檻 + ZYGOSITY 取哪個 caller 的 GT、性染色體依 GT 套數 | ZYGOSITY 把 HC 的 call 標成 ref；女性 chrX 的 `1/1` 被標成 hemizygous（5.4） |
| `test_parse_pgx_mtrn1.py` | `parse_pgx_report.parse_mito_tsv()` | **9 個案例** | MT-RNR1 coverage 閘門失效（10.12） |
| `test_add_callers_tag.py` | `add_callers_tag.py` | **8 個案例** | 兩邊都沒 call 被標 HC、AD 缺值補 0（3.3） |
...
python3 scripts/test_add_callers_tag.py
```

## 12.1 `test_combine_phased.py`（26 個案例）

這是五支測試中案例最多的一支 —— 它守住的是二級大補帖 7.6 節那個
「AD 消失」的嚴重 bug、7.6.1「DV 否決的候選被挑成 anchor」、7.6.2「phase 未知的重疊 het 被當成同一條單體」不會再發生。
（二級、三級兩份 byte-identical，md5 需一致。）
...
- 未 phase（或 PS 不同）的 het → **不合**，即使重疊（2026-09 起；舊版重疊就合）
- ref-free trim 移除 padding（避免假性重疊）
- **合成記錄保留 `AD`/`DP`/`VAF`**（anchor 繼承）
- 原封通過的記錄保留原本的 `AD`
- 孤立變異完全不動
- **GT 沒有 ALT 的紀錄不進叢集**（2026-09 新增 3 例，舊版都會失敗）：
  - 被否決的較寬缺失（`./.`、RefCall）蓋住真的 SNV → 兩筆都原封通過，SNV 保留自己的 AD
    （舊版：SNV 被改寫成 `CGTA>CGGA`，帶著缺失候選的 FILTER=RefCall / AD / VAF）
  - 被否決的長缺失橫跨兩顆相距 4 bp 的 cis SNV → 不會被串成一個 MNV
  - 單套的 `0` / `.` 也不參與叢集
- **合成結果輸出前最小化**（2026-09，舊版會失敗）：一邊是「插入（含前導鹼基）+ SNV」、另一邊是一筆最小表示的
  同一個變異 → 合成後兩邊都是 `101 C>TG`（舊版寫成 `100 AC>ATG`，merge 合不起來）；純缺失保留 anchor 鹼基
- **phase 未知或重疊互相矛盾 → 原封通過**（2026-09 新增 7 例，舊版全部失敗；VAL-10 有 7,927 個 allele 因此消失）：
  - 兩個重疊的 het 缺失、沒有 PS（或 PS 不同）→ 不再併成 5 bp 的缺失
  - SUZ12 形狀的 del+ins 但沒有 phase → 兩筆原封（有共同 PS 時照樣合成）
  - 缺失範圍內的 SNV、hom 缺失裡的 het 缺失、錨在缺失中間的插入、`*` allele → 不吃掉、不截斷
  - 同一個鹼基上的 SNV + 插入 → `A>GT`（舊版先套插入，SNV 被吃掉）；SNV + 缺失 → `AC>G`
...
> 更正（2026-09）：本節舊版寫「涵蓋 `pick_transcripts_for_output()` 與 `SLC37A4`」，
> 但這支測試從來沒有測轉錄本選取；實際內容如下。轉錄本選取目前**沒有**單元測試。

- `strand_bias_flag()`：SNV `FS>60 / SOR>3.0`、indel `FS>200 / SOR>10.0` 的門檻；
  DeepVariant-only 位點沒有 FS/SOR → `.`；`STRAND_BIAS` 有登記在輸出欄位。
- `infer_zygosity()`（2026-09 新增）：
  - `test_zygosity_uses_the_caller_that_called`（舊版會失敗）：DV `0/0` + HC `1/0` → `het`、
    DV `0|0` + HC `1/1` → `hom`、男性 chrX DV `0` + HC `1` → `hemizygous`、DV `./1` + HC `1/1` → `hom`
  - `test_zygosity_unchanged_cases`（舊版也通過，確認其他情況行為不變）：兩邊都有 call 時 DV 優先、
    DV 缺值用 HC、DRAGEN（`gt_hc` 為 `.`）、兩邊都沒 call 時沿用舊邏輯
  - `test_zygosity_sex_chromosomes_by_ploidy`（VAL-10 之後新增，舊版會失敗）：女性 chrX `1/1`、`1|1` → `hom`、
    男性 PAR `1/1` → `hom`、chrX `1/2` 與半缺失 `1/.` → `het`；單套 `1`（男性 chrX 非 PAR、chrY）仍是 `hemizygous`
- `haploid_het_callers()`（2026-09 新增，`test_haploid_het_column`）：`HAPLOID_HET` 只留這一筆真的有 call 的
  caller（多等位拆開後，原本是 het 的可能只是其中一個 allele）；`HAPLOID_HET` 是最後一欄，既有欄位位置不變
...
## 12.5 `test_add_callers_tag.py`（8 個案例）

守住 3.3 的 SUZ12 案例。核心不變式：

> `CALLERS` 只有在某個 caller **有完整的 ALT genotype** 時才不是 `NONE`；
> `NONE` 不在 `FILTER_FOR_ANNOTATION` 允許的清單裡；AD 的個別缺值保留 `.`，
> 真正的 0 仍然是 0。

| 案例 | 輸入 | 期望 |
|------|------|------|
| `test_is_called` | `0/1` `0\|1` `1/1` `1/0` `0\|2` vs `0/0` `0\|0` `./.` `./1` `1/.` | 前五個 called；後五個不是（半缺失也不算）|
| **`test_suz12_rejected_allele_is_none`** | DV `./.` + HC `0\|0`（c.2170del 拆出來的那筆）| **`NONE`**（舊版 `HC`）|
| `test_neither_called_is_none` | `./.`+`./.`（952/953 A>T）、`0/0`+`./.`（DV RefCall）| `NONE`（舊版 `HC`）|
| `test_real_calls_unchanged` | 真的 HC-only、DV+HC、DV-only | 分類與舊版相同 |
| `test_none_excluded_by_real_filter` | **讀真正的 `prepare_vcf.nf`**，解析 `-i` 條件 | 允許清單 = `{DV+HC, DV, HC}`，不含 `NONE` |
| `test_get_ad_keeps_missing` | `10,<miss>`、`3,<miss>`、`10,14,<miss>` | `10,.` / `3,.` / `10,14,.`（舊版補 0）|
| `test_get_ad_real_values_unchanged` | `3,2`、`30,0`、全缺值、無 AD 欄 | `3,2` / `30,0` / `.` / `.` |
| `test_suz12_locus_end_to_end` | SUZ12 那四筆拆分後的紀錄 | **只有 delinsTT 通過**，`CALLERS=HC`、`AD_DV=10,.`、`AD_HC=3,2`（舊版四筆全過）|

兩個設計值得一提：

- **`test_none_excluded_by_real_filter` 直接讀 `prepare_vcf.nf`** 的 `-i` 條件，
  而不是在測試裡寫死允許清單。所以將來有人改了 CALLERS 的詞彙、或不小心把 `NONE`
  加進過濾條件，測試會直接失敗 —— 程式碼與過濾條件不會悄悄脫節。
- **已驗證這支測試對舊版程式碼會失敗**（在 `test_suz12_rejected_allele_is_none`
  那一行），否則它就沒有守住任何東西。

cyvcf2 沒安裝時會自動 stub（測試只用純函式 + 假的 variant 物件）；有安裝時用真的，
兩種情況都已實測通過。
...
## A.1 `03_acmg/<sample>.snv_indel.acmg.tsv`（82 欄，主要交付物）

> 欄號 1 起算（與 `awk '{print $N}'` 相同）。2026-09 核對過程式的 `OUTPUT_COLUMNS` + ACMG 新增欄：
> 舊版從 in-silico 那列起少算 1。新欄位一律附加在最後，但用欄位名稱查找比寫死欄號安全。
...
| 42–56 | in-silico 分數 | BayesDel / AlphaMissense / ESM1b / VARITY / SIFT / DANN / PHACTboost / phyloP / GERP / **P-KNN LLR + EVIDENCE** |
| 57–58 | `PANGOLIN_SCORE` `PANGOLIN_DETAIL` | 剪接預測 |
| 59–60 | `DOMAINS` `SWISSPROT` | 蛋白質功能域 |
| 61 | `HGNC_ID` | |
| 62–64 | `CLINGEN_VCEP_CLASS/_CRITERIA/_PANEL` | **專家判讀（對照用，不計分）** |
| 65–71 | `REVEL` `MUTPRED2` `MUTPRED2_PRED` `VEST4` `CADD_PHRED` `GNOMAD41_JOINT_AF` `_EAS_AF` | **5.3a 專屬（參考用，不計分）** |
| 72 | `DBNSFP_VERSION` | `4.9c` 或 `5.3a` |
| 73–74 | `NMD` `PROTEIN_POSITION` | PVS1 決策樹用 |
| 75 | `HAPLOID_HET` | 男性 chrX 非 PAR 原本叫成 het 的 caller（`DV` / `HC` / `DV,HC`；`.` = 無）→ **需人工複核**（嵌合／XXY／比對假象）|
| — | *（以上 75 欄由 `parse_vep_csq.py` 產生）* | |
| 76–79 | `ACMG_CRITERIA` `ACMG_SCORE` `ACMG_CLASS` `ACMG_NOTES` | **ACMG 分類與理由** |
| 80 | `CLINGEN_AGREEMENT` | `AGREE` / `DIFFER_TIER` / `DIFFER` / `.` |
| 81–82 | `PVS1_STRENGTH` `PVS1_REASON` | **PVS1 決策樹走了哪條路** |
...
| 有單元測試嗎？ | 五支，13 + 9 + 8 + N 個案例，每一支都對應一個已修過的嚴重 bug，且都驗證過對舊版程式碼會失敗 | 第 12 章 |
| 已知的限制有哪些？ | PVS1 三個簡化（6.3）、STR 超長擴增只能報下限（8.6）、5 條 ACMG 準則未實作（6.6）、dbNSFP 4.9c 的 CSV 解析缺陷（11.7，已修） | 各章 |
| 有沒有自己抓到過臨床風險的 bug？ | 有，兩個。**MT-RNR1 coverage 閘門失效**——沒測到的樣本會被報成「可正常用 aminoglycoside」。逐行核對原始碼時發現，2026-08 修正（閘門生效＋只認三個致病位點＋報告只列實際評估過的變異），7 個情境重測通過 | 10.12 |
| （續）第二個是怎麼發現的？ | **與 DRAGEN 交叉比對**時發現。SUZ12 的 delinsTT，NCKUH 多出一個 DRAGEN 沒有的錯誤 `c.2170del`，而正確的那一列顯示 VAF 0。根因是三處各自的小錯疊加：ensemble merge 把 DV 否決的候選併進 HC 的 call、`determine_callers` 把「兩邊都沒 call」標成 HC、`get_ad` 把缺值補成 0。2026-09 三處都修，用 pysam 的 bcftools 逐欄重現確認、補回歸測試 | 3.3、12.5 |
| （續）修完之後有再驗證嗎？ | 有，VAL55 整套重跑：幽靈列 0、SUZ12 只剩 delinsTT。同一次驗證又抓到兩個問題並修正：`combine_phased.py` 把 DV 否決的候選挑成 anchor（ensemble 殘留 28,050 筆 RefCall，23,023 個變異在報告裡被拆成兩列）、ZYGOSITY 取到 DV 的 `0/0` 把 HC 的 call 標成 ref（15,422 列）。兩者都有回歸測試、舊版會失敗；修正後重跑：RefCall 28,050 → 0、拆兩列 23,023 → 820、ref 15,422 → 0 | 3.3.1、12.1、12.3 |
| （續）DRAGEN 路徑呢？ | VAL-10（女性 WGS）重跑：新版生效、AD 與 7 月相同、性別核對（ploidy QC、chrX GT、chrX STR）全部一致。又抓到兩個問題並修正：combine 讓 non-PASS 紀錄參與叢集、合成紀錄沿用 anchor 的 FILTER → PASS 變異可能被丟、濾掉的 allele 可能混入（改成只拿 PASS 進 combine）；女性 chrX 的 `1/1` 被標成 hemizygous（改看 GT 的套數）。重跑後逐叢檢查再抓到兩項：phase 未知的重疊 het 被當成同一條單體（7,927 個 PASS allele 消失；改成 phase 未知或重疊矛盾就不合）、拆多等位後樣本沒帶的 allele 變成 `0/0` 假列（norm 後只留有 ALT 的）。都有 toy／回歸測試 | 3.3.2、3.6、5.4、12.1、12.3 |
| 這兩個 bug 有什麼共同點？ | 都是**註解寫著正確的設計意圖、程式碼卻沒做到**，而且沒有測試去驗證那個意圖。修正方式也相同：把意圖變成會失敗的測試 | 3.3、10.12 |
