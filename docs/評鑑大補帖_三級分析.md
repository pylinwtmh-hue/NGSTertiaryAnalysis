...
| 測試 | 對象 | 涵蓋 | 守住的 bug |
|------|------|------|-----------|
| `test_combine_phased.py` | `combine_phased.py` | **19 個案例** | 合成記錄 AD 消失、否決候選被當 anchor、合成結果未最小化（二級大補帖 7.6 / 7.6.1 / 7.5） |
...
## 12.1 `test_combine_phased.py`（19 個案例）

這是五支測試中案例最多的一支 —— 它守住的是二級大補帖 7.6 節那個
「AD 消失」的嚴重 bug，以及 7.6.1「DV 否決的候選被挑成 anchor」不會再發生。
...
| （續）DRAGEN 路徑呢？ | VAL-10（女性 WGS）重跑：新版生效、AD 與 7 月相同、性別核對（ploidy QC、chrX GT、chrX STR）全部一致。又抓到兩個問題並修正：combine 讓 non-PASS 紀錄參與叢集、合成紀錄沿用 anchor 的 FILTER → PASS 變異可能被丟、濾掉的 allele 可能混入（改成只拿 PASS 進 combine）；女性 chrX 的 `1/1` 被標成 hemizygous（改看 GT 的套數）。兩者都有 toy／回歸測試 | 3.3.2、3.6、5.4、12.3 |
