...
profiles {
    ...
    dgm {
        params {
            ...
            pangolin_num_gpus   = 1
            // DGM 是 RTX A2000（GA106 / Ampere）= sm_86，cu121 與 cu128 兩顆都含 sm_86
            //   → 兩顆都能跑。這裡跟 local 共用 cu128 那顆，少維護一條部署路徑。
            //   （佐證：DGM 原本跑舊的 cu130 容器就正常，而 cu128 的 arch list
            //     sm_75…sm_120 比 cu130 的 sm_75 sm_80 sm_90 sm_100 sm_120 還寬，
            //     需要的驅動又更舊 → 跑得動 cu130 就一定跑得動 cu128。）
