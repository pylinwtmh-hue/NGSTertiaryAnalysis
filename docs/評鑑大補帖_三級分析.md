...
DV+HC 89.0%、DV only 3.0%、**HC only 7.8%**、NONE 0.2%（推測多為男性 chrX/Y 被 `+fixploidy`
截成 REF 的 het，待依染色體確認）。
...
剩下 820 個推測是兩個 caller 對同一變異寫法不同、三級正規化後才對齊的情況（待看實例確認）。
...
| 測試 | 對象 | 涵蓋 | 守住的 bug |
|------|------|------|-----------|
| `test_combine_phased.py` | `combine_phased.py` | **18 個案例** | 合成記錄 AD 消失、否決候選被當 anchor（二級大補帖 7.6 / 7.6.1） |
...
## 12.1 `test_combine_phased.py`（18 個案例）
