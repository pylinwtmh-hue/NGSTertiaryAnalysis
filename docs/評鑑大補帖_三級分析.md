...
profiles {
    ...
    local {
        params {
            ...
            pangolin_num_gpus   = 1
            // 開發機是 RTX PRO 6000 Blackwell（sm_120）→ 用 cu128 那顆。
            //   ⚠️ 這顆**不能**拿去 DGX-2：cu128 wheel 已砍掉 sm_70，V100 會噴
            //      "no kernel image is available for execution on the device"。
            pangolin_sif        = "pangolin_1.0.0.sif"
    ...
    dgm {
        params {
            ...
            //   DGM 是 RTX A2000（GA106 / Ampere）= sm_86，cu121（sm_50…sm_90）與
            //   cu128（sm_75…sm_120）兩顆都含 sm_86，所以兩顆都跑得動。
            ...
            pangolin_sif        = "pangolin_v100_1.0.0.sif"
    ...
    dgx {
        params {
            ...
            use_gpu_lock        = true
            pangolin_num_gpus   = 1
            // ⚠️ DGX-2 專用容器：V100 是 sm_70，只有 cu121 的 wheel 有 sm_70 kernel。
            //    開發機用的 pangolin_1.0.0.sif（cu128）在這裡會噴
            //    "no kernel image is available for execution on the device"。
