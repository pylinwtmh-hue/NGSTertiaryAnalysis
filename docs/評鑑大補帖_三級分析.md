...
> STR 超長擴增只能報下限（8.6）、**MT-RNR1 coverage 閘門目前失效（10.12）**、
> dbNSFP 4.9c 檔案帶有已修正的 CSV 解析缺陷（11.7）。
...
三個位點（`chrM:1555`、`chrM:1494`、`chrM:827`）**設計上**有三種狀態：

| mito.tsv 有致病變異？ | mpileup 有覆蓋（DP≥10）？ | 應得結論 |
|---------------------|------------------------|---------|
| 有 | — | `mtrn1_risk=HIGH` → aminoglycoside 禁用 |
| 沒有 | 有 | `mtrn1_risk=LOW` → 確認野生型，可正常用藥 |
| 沒有 | 沒有 | **不輸出任何列 → 顯示 Unknown**（不能當成野生型） |

**為什麼一定要區分後兩者**：若把「沒有覆蓋」當成「野生型」，
會讓一個實際上帶 m.1555A>G 的病人被判成「可以用 gentamicin」——
造成不可逆的聽力喪失。這是 PGx 中風險最高的誤判之一。

### ⚠️ 已知缺陷：coverage 閘門目前失效（`parse_mito_tsv`）
...
**實際行為**：只要 `mito.tsv` 存在且裡面沒有 MT-RNR1 致病變異，
就一律輸出 `Reference / mtrn1_risk=LOW`，**即使三個位點的深度都是 0**。
`PGX_MTRN1` 的 mpileup 目前只影響 `notes` 欄的文字
（有覆蓋 → 列出位點；無覆蓋 → 寫 "Coverage confirmed via mito pipeline"，
而這句話在無覆蓋時是**不正確的**）。

**臨床影響**：若某樣本的 chrM 三個位點覆蓋不足，報告會顯示
「無 MT-RNR1 風險變異，可正常使用 aminoglycoside」，
但實際上是「沒測到，不知道」。這正是上面說要避免的誤判方向。

**修正方式**（一行）：把 `or no_mito is False` 拿掉 ——

```python
if covered_positions:            # 只有真的有覆蓋才補 Reference 列
```

這樣「有 mito.tsv 但三個位點都沒覆蓋」就會落到「不輸出列 → Unknown」，
與設計意圖一致。

> 此缺陷已寫入本文件供評鑑說明；**尚未修改程式碼**（超出本次文件工作的範圍，
> 且屬臨床判讀邏輯變更，應先與臨床端確認後再改並重跑驗證樣本）。
...
三支測試腳本，全部只用 Python 標準庫的 `unittest`，不需要真實資料：

| 測試 | 對象 | 涵蓋 |
|------|------|------|
| `test_combine_phased.py` | `combine_phased.py` | **13 個案例** |
| `test_parse_dragen_ploidy.py` | `parse_dragen_ploidy.py` | 解析 + 判定 + 警告 |
| `test_parse_vep_csq.py` | `parse_vep_csq.py` | 轉錄本選取邏輯 |
...
| 有單元測試嗎？ | 三支，13 + N 個案例，守住已修過的嚴重 bug | 第 12 章 |
| 已知的限制有哪些？ | PVS1 三個簡化（6.3）、STR 超長擴增只能報下限（8.6）、**MT-RNR1 coverage 閘門缺陷（10.12）**、5 條 ACMG 準則未實作 | 各章 |
