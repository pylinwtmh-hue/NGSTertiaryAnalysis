...
修正後需重新量測；新版 stderr 會把 `NONE` 單獨列出。
...
DV+HC 89.0%、DV only 3.0%、**HC only 7.8%**、NONE 0.2%（多為男性 chrX/Y 被 `+fixploidy`
截成 REF 的 het）。
...
def add_callers_tag(input_path: str, sample_id: str, output_path: str):
    ...
    #        中，HC-only 包含了 NONE（見上方統計區塊），**這組數字需重新量測**。
...
| （續）修完之後有再驗證嗎？ | 有，VAL55 整套重跑：幽靈列 0、SUZ12 只剩 delinsTT。同一次驗證又抓到兩個問題並修正：`combine_phased.py` 把 DV 否決的候選挑成 anchor（ensemble 殘留 28,050 筆 RefCall，23,023 個變異在報告裡被拆成兩列）、ZYGOSITY 取到 DV 的 `0/0` 把 HC 的 call 標成 ref（15,422 列）。兩者都有回歸測試、舊版會失敗 | 3.3.1、12.1、12.3 |
